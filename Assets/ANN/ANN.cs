...
public class ANN{
	...
	public ANN(int numInputs, int numOutputs, int numHidden, int numNPerHidden, double alpha)
	{
		...
		if(this.numHidden > 0)
		{
            layers.Add(new Layer(this.numNPerHidden, this.numInputs));

			for(int i = 0; i < this.numHidden-1; i++)
			{
                layers.Add(new Layer(this.numNPerHidden, this.numNPerHidden));
			}

            layers.Add(new Layer(this.numOutputs, this.numNPerHidden));
		}
		else
		{
            layers.Add(new Layer(this.numOutputs, this.numInputs));
		}
	}
...
/*
 This ANN is multipurpose
 So far has been used to solve logic gates and pong 
     */